:Namespace HTTPTools
    ...
    ∇ r←CachedGet url
      :If 0=⎕NC'#.WWWCache' ⍝ Cache exists?
          (#.WWWCache←⎕NS'').((Keys Data)←⍬ ⍬)
      :EndIf
     
      :If 0=⍴r←WWWCacheRead url
          r←fromUTF8 GetPage url
          r WWWCacheUpdate url
      :EndIf
    ∇
    
    ∇ r←text WWWCacheUpdate url;index
      index←#.WWWCache.Keys⍳⊂url
      :If index≤≢#.WWWCache.Keys ⍝ key found
          #.WWWCache.Data[index]←⊂text
      :Else ⍝ new key
          #.WWWCache.Keys,←⊂url
          #.WWWCache.Data,←⊂text
      :EndIf
    ...
    ∇ r←WWWCacheRead url
      r←(#.WWWCache.Keys⍳⊂url)⊃#.WWWCache.Data,⊂''
    ∇
